...
bool print_winner(void)
{
    ...
    for (int k = 0; k < candidate_count; k++)
   {
        if (candidates[k].votes >= maj)
        {
            if (candidates[k].votes == candidates[k].votes)
            {
...
int find_min(void)
{
    ...
    {
     if (candidates[f].eliminated == 0)
        {
            if (candidates[f].votes < min)
            {
             min = candidates[f].votes;
            }
